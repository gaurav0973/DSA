//{ Driver Code Starts
import java.util.*;
import java.io.*;
import java.lang.*;

class DriverClass
{
    public static void main(String args[]) throws IOException {

        BufferedReader read =
            new BufferedReader(new InputStreamReader(System.in));
        int t = Integer.parseInt(read.readLine());
        while (t-- > 0) {
            String str[] = read.readLine().trim().split(" ");
            int V = Integer.parseInt(str[0]);
            int E = Integer.parseInt(str[1]);
    
            ArrayList<ArrayList<ArrayList<Integer>>> adj = new ArrayList<ArrayList<ArrayList<Integer>>>();
            for(int i=0;i<V;i++)
            {
                adj.add(new ArrayList<ArrayList<Integer>>());
            }
            
            int i=0;
            while (i++<E) {
                String S[] = read.readLine().trim().split(" ");
                int u = Integer.parseInt(S[0]);
                int v = Integer.parseInt(S[1]);
                int w = Integer.parseInt(S[2]);
                ArrayList<Integer> t1 = new ArrayList<Integer>();
                ArrayList<Integer> t2 = new ArrayList<Integer>();
                t1.add(v);
                t1.add(w);
                t2.add(u);
                t2.add(w);
                adj.get(u).add(t1);
                adj.get(v).add(t2);
            }
            
            int S = Integer.parseInt(read.readLine());
            
            Solution ob = new Solution();
            
            int[] ptr = ob.dijkstra(V, adj, S);
            
            for(i=0; i<V; i++)
                System.out.print(ptr[i] + " ");
            System.out.println();
        }
    }
}
// } Driver Code Ends


//User function Template for Java
class Pair {
    int first;
    int second;
    Pair(int f, int s){
        this.first = f;
        this.second = s;
    }
}

class Solution {
    // Function to find the shortest distance of all the vertices
    // from the source vertex S.
    static int[] dijkstra(int V, ArrayList<ArrayList<ArrayList<Integer>>> adj, int s) {
        // 1. creation of minHeap
        PriorityQueue<Pair> pq = new PriorityQueue<>((a, b) -> a.first - b.first);

        // 2. distance array initialised to infinity
        int[] dis = new int[V];
        Arrays.fill(dis, Integer.MAX_VALUE);

        // 3. scr to source distance is zero and add in pq
        dis[s] = 0;
        pq.add(new Pair(0, s)); // (distance, node)

        // 4. level order in minHeap
        while (!pq.isEmpty()) {
            Pair rem = pq.remove();
            int distance = rem.first;
            int node = rem.second;

            for (ArrayList<Integer> nbr : adj.get(node)) {
                int adjNode = nbr.get(0);
                int edgeWt = nbr.get(1);

                if (distance + edgeWt < dis[adjNode]) {
                    dis[adjNode] = distance + edgeWt;
                    pq.add(new Pair(dis[adjNode], adjNode));
                }
            }
        }
        return dis;
    }
}

